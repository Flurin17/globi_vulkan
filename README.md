...
pnpm test:fountain-gpu # Native GPU check; requires uv and OpenGL 3.3
```
...
The GLB contains the reusable static model; `src/components/ProductScene.tsx` assembles the scene and `src/components/Fountain.tsx` renders the effect. Studio environment lighting and soft shadows reveal the paper finish. The fountain is based on five-second frame samples of the [original Globi-Vulkan video](https://www.youtube.com/watch?v=yGuuYAInBBg), including its final frames. A condensed 28-second cycle moves from green-white ignition to a taller gold jet with silver crackle, reaching full height in roughly 2.5 seconds, then falling embers and residual smoke. Particle flight uses real seconds, linear air resistance and gravity; the burn envelope is sampled at launch so burnout does not collapse airborne trajectories. GPU-instanced ribbons give sparks continuous tapered streaks, cooling colours and occasional glints. Separate noisy smoke and jet layers add depth. Narrow canvases use 1,716 spark instances and 30 smoke puffs; wider canvases use 3,429 and 48. No video assets are loaded by the animation. Static phases render on demand when scroll progress changes. The fuse and fountain request continuous frames only while active; rendering pauses when the scene leaves the viewport, the tab is hidden, or the visitor presses pause. The 3D bundle starts after the poster has painted and the browser is idle; data-saver visitors retain the static poster. Reduced-motion preference, WebGL unavailability, context loss and model-load failures show the poster instead.

The optional GPU regression check compiles the shipped spark shader and renders its actual plane geometry with back-face culling enabled. It checks eight desktop/mobile/time combinations for visible output and verifies full plume height at 2.5 seconds and confirms that enabling culling preserves every spark. This catches shader orientation errors that shader compilation and ordinary unit tests cannot detect.
